...
```{r}
main_libs <- read_csv('data/Main_Libraries__Branches__and_Bookmobiles__FY_2014_Public_Libraries_Survey__Outlet_Data_.csv')
...
```{r}
main_libs <- main_libs %>% 
  rename(fscs_id = `FSCS ID`, fscs_id_seq = "FSCS ID_SEQ", library_id = "LIBRARY ID", library_name = "LIBRARY NAME", county_pop = "COUNTY POPULATION", outlet_type = "OUTLET TYPE", sq_feet = "SQ FEET", hours = HOURS, weeks_open = "WEEKS OPEN", metro_status = "METRO STATUS", structure_change = "STRUCTURE CHANGE", name_chage = "NAME CHANGE", address_change = "ADDRESS CHANGE")
```

```{r}
main_libs %>% 
  select(county_pop, sq_feet, hours) %>% 
  head() %>% 
  kable(
    digits=2,
