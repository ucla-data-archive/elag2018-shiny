...
### Histograms via geom_histogram 
 ...
 The histogram shows how many elements of a single numerical variable fall in specified *bins*.  In this case, these bins may correspond to between 0-10&deg;F, 10-20&deg;F, etc. We produce a histogram of the hour temperatures at all three NYC airports in 2013:
...
The center is around `r mean(la_circ$circulation, na.rm=TRUE)`&deg;F. By running the `summary()` command, we see that the median is about 3000 less than the mean, typical of a skewed right variable. 
